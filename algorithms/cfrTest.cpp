...
TEST(CFR, CheckConvergenceSmallStepsInSmallDomain2) {
    ...
    CFRAlgorithm cfr(domain, Player(0), data, settings);
    double expectedUtilities[] =
        {-0.25, -0.125, -0.0833333333, -0.0625, -0.05, -0.0416666667, -0.0357142857, -0.03125,
         -0.0277777778,
         -0.025};
    double expectedBestResp0[] =
        {0.5, 0.25, 0.166666667, 0.125, 0.1, 0.0833333333, 0.0714285714, 0.0625, 0.0555555556,
         0.05};
    double expectedBestResp1[] =
        {0.25, 0.125, 0.0833333333, 0.0625, 0.05, 0.0416666667, 0.0357142857, 0.03125, 0.0277777778,
         0.025};
...
TEST(CFR, CheckExploitabilityInSmallDomain) {
    ...
    cfr.runIterations(2);
    auto profile = getAverageStrategy(data);
    EXPECT_LE(std::fabs(0.159173 - calcExploitability(*domain, getAverageStrategy(data)).expl),
              0.0001);
}
