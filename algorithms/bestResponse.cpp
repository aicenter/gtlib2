...
bestResponseTo(const BehavioralStrategy &opoStrat, int opponent, int player, const Domain &domain) {
...
pair<BehavioralStrategy, double> bestResponseTo(const BehavioralStrategy &opoStrat,
        ...
        if (*node->getCurrentPlayer() == player) {
          ...
          for (const auto &action : node->availableActions()) {
            ...
            for (const auto &siblingNatureProb : allNodesInTheSameInfSet) {
              ...
              double seqProb = sibling->getProbabilityOfActionsSeqOfPlayer(opponent,
                                                                           opoStrat);
              double val = 0;
  ...
  auto initNodesProb = createRootEFGNodesFromInitialOutcomeDistribution(
...
pair<BehavioralStrategy, double>
bestResponseToPrunning(const BehavioralStrategy &opoStrat, int opponent, int player,
                       const Domain &domain) {
...
pair<BehavioralStrategy, double> bestResponseToPrunning(
    const BehavioralStrategy &opoStrat, const int opponent, const int player,
      ...
      [&player, &opponent, &domain, &bestResp, &nodesInInfSet, &opoStrat, &cache]
        ...
        if (*node->getCurrentPlayer() == player) {
          ...
          if (nodesInInfSet.find(infSet) != nodesInInfSet.end()) {
            allNodesInTheSameInfSet = nodesInInfSet[infSet];
          } else {
            allNodesInTheSameInfSet =
                getAllNodesInTheInformationSetWithNatureProbability(infSet, domain);
          }
          ...
          for (const auto &action : node->availableActions()) {
            ...
            for (const auto &siblingNatureProb : allNodesInTheSameInfSet) {
              ...
              for (auto siblingProb : sibling->performAction(action)) {
                auto brs_val = bestResp(siblingProb.first,
                                        siblingProb.first->getState() ==
                                            sibling->getState() ? depth : depth - 1,
                                        natureProb * seqProb * siblingProb.second);
                val += brs_val.second;
                brs.insert(brs_val.first.begin(), brs_val.first.end());
              }
          ...
          for (const auto &action : node->availableActions()) {
            if (stratAtTheNode.find(action) != stratAtTheNode.end()) {
              ...
              for (auto childProb : node->performAction(action)) {
                auto brs_val = bestResp(childProb.first,
                                        childProb.first->getState() ==
                                            node->getState() ? depth : depth - 1,
                                        prob * childProb.second * actionProb);
                val += brs_val.second;
                brs.insert(brs_val.first.begin(), brs_val.first.end());
              }
