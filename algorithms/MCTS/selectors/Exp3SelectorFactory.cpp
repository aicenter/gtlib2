...
namespace GTLib2::algorithms {
    ...
    unique_ptr<Selector> Exp3SelectorFactory::createSelector(int actionsNumber) {
        if (useExp3L) return make_unique<Exp3LSelector>(actionsNumber, this);
        else return make_unique<Exp3Selector>(actionsNumber, this);
    }

    unique_ptr<Selector> Exp3SelectorFactory::createSelector(vector<shared_ptr<Action>> actions) {
        if (useExp3L) return make_unique<Exp3LSelector>(actions, this);
        else return make_unique<Exp3Selector>(actions, this);
    }

    std::mt19937 Exp3SelectorFactory::getRandom() {
        return generator_;
    }

    double Exp3SelectorFactory::normalizeValue(double value) {
