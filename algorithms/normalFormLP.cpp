...
namespace GTLib2 {
    ...
    NormalFormLP::NormalFormLP(const shared_ptr<Domain> _game,
                               shared_ptr<LPSolver> _lp_solver) {
        lp_solver = std::move(_lp_solver);
        auto aohistories = vector<shared_ptr<unordered_map<shared_ptr<InformationSet>,
                vector<shared_ptr<Action>>>>>
                {make_shared<unordered_map<shared_ptr<InformationSet>,
                        vector<shared_ptr<Action>>>>(),
                 make_shared<unordered_map<shared_ptr<InformationSet>,
                         vector<shared_ptr<Action>>>>()};

        std::function<void(EFGNode *, vector<shared_ptr<unordered_map<shared_ptr<InformationSet>,
