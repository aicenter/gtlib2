...
void CplexLPSolver::BuildModel(int rows, int cols,
                               const vector<double>* utility_matrix,
                               const bool OUTPUT) {
  env_ = IloEnv();
  model_ = IloModel(env_);

  x_ = IloNumVarArray(env_, cols, 0, 1, ILOFLOAT);
...
void CplexLPSolver::SetConstraintCoefForVariable(int constraint, int variable,
                                                 double new_utility) {
