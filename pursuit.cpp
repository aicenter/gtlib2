...
PursuitObservation::PursuitObservation(int id, int move):
        Observation(id), move_(move) {}
...
ProbDistribution PursuitState::PerformAction(vector<shared_ptr<Action>> actions2) {
    // TODO(rozlijak): one-line - casting to PursuitAction
    vector<shared_ptr<PursuitAction>> actions = vector<shared_ptr<PursuitAction>>();
    for (const auto &j : actions2) {
        actions.push_back(std::dynamic_pointer_cast<PursuitAction>(j));
    }
...
string PursuitDomain::GetInfo() {
    return "Rozmery pole jsou: " + to_string(PursuitDomain::height_) + " x " +
           to_string(PursuitDomain::height_) + " maximalni hloubka grafu je: " +
...
//        cout << "    ";
//    }
//    string s = to_string(state->getPlace()[0].x) + " " +
//            to_string(state->getPlace()[0].y) + "    " +
//               to_string(state->getPlace()[1].x) + " " +
//            to_string(state->getPlace()[1].y);
//      cout << s<< '\n';
    vector<shared_ptr<Action>> actions = vector<shared_ptr<Action>>();
    vector<shared_ptr<Action>> actions2 = vector<shared_ptr<Action>>();
    ++count;
    if (depth == 0) {
        return;
    }
    ...
    for (auto &action : actions) {
        for (auto &j : actions2) {
            vector<shared_ptr<Action>> base = {action, j};
         //   cout << j.use_count() << "    ";
            // TODO(rozlijak): change base to unique pointers
