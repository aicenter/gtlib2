...
void NormalFormLP::BuildModel() {
    assert(utility_matrix_ != nullptr);
    ...
    for (int i=0; i<rows_; ++i) {
        IloExpr sum(env_);
        for (int j=0; j<cols_; ++j) {
            sum += utility_matrix_->at(i*cols_+j)*x_[j];
        }
    ...
    model_ready_ = true;
    utility_matrix_ = nullptr; // clearing the pointer
}
