...
RhodeIslandPokerDomain::RhodeIslandPokerDomain(unsigned int maxCardTypes, unsigned int maxCardsOfTypes,
                                       unsigned int maxRaisesInRow, unsigned int maxDifferentBets,
                                       unsigned int maxDifferentRaises, unsigned int ante):
        Domain(9+2*maxRaisesInRow, 2), maxCardTypes(maxCardTypes), maxCardsOfEachType(maxCardsOfTypes),
        maxRaisesInRow(maxRaisesInRow), ante(ante), maxDifferentBets(maxDifferentBets),
    ...
    for(int color2 = 0; color2 < maxCardsOfEachType; ++color2) {
      for (int p1card = 0; p1card < maxCardTypes; ++p1card) {
        for (int p2card = 0; p2card < maxCardTypes; ++p2card) {
          ...
          auto newState = make_shared<RhodeIslandPokerState>(this, make_pair(p1card, color),
                  make_pair(p2card, color2), nullopt, nullopt, ante, next_players);
          vector<shared_ptr<Observation>> newObservations{make_shared<RhodeIslandPokerObservation>
                                                                  (3 + p1card+color*maxCardTypes, PlayCard, p1card, color),
                                                          make_shared<RhodeIslandPokerObservation>(
                                                                  3 + p2card+color2*maxCardTypes, PlayCard, p2card, color2)};
...
RhodeIslandPokerDomain::RhodeIslandPokerDomain(): RhodeIslandPokerDomain(4, 3, 1, 2, 2, 1) {}
...
string RhodeIslandPokerDomain::getInfo() const {
  ...
  std::copy(betsSecondRound.begin(), betsSecondRound.end(), std::ostream_iterator<int>(bets3, ", "));
  ...
  std::copy(raisesSecondRound.begin(), raisesSecondRound.end(), std::ostream_iterator<int>(raises3, ", "));
...
RhodeIslandPokerState::performActions(const vector<pair<int, shared_ptr<Action>>> &actions) const  {
  auto pokerDomain = dynamic_cast<RhodeIslandPokerDomain*>(domain);
  auto a1 = dynamic_pointer_cast<RhodeIslandPokerAction>(std::find_if( actions.begin(), actions.end(),
          [](pair<int, shared_ptr<Action>> const & elem) { return elem.first == 0; })->second);
  auto a2 = dynamic_pointer_cast<RhodeIslandPokerAction>(std::find_if( actions.begin(), actions.end(),
  ...
  if (a1) {
    switch (a1->GetType()) {
      case Raise:
        ...
        switch (round_) {
          ...
          case 3: raisePosition = static_cast<int>(pokerDomain->maxDifferentRaises);
          ...
          case 5: raisePosition = 2*static_cast<int>(pokerDomain->maxDifferentRaises);
        ...
        bet = lastAction->GetValue() + a1->GetValue(); //2 *(pot - firstPlayerReward) - pot + a1->GetValue();
        new_pot += bet;
        break;
      ...
      case Bet:
        ...
        switch (round_) {
          case 1:
            ...
            break;
          case 3: betPosition = static_cast<int>(pokerDomain->maxDifferentBets);
            ...
            break;
          case 5: betPosition = 2*static_cast<int>(pokerDomain->maxDifferentBets);
        ...
        id += betPosition;
        break;
      case Fold: id = Fold; newFirstPlayerReward = pot-firstPlayerReward; new_round = pokerDomain->TERMINAL_ROUND; break;
      default: break;
    }
    if (new_round == 2 && natureCard1_ == nullopt && a1->GetType() == Call) {
      ...
      for(int j = 0; j < pokerDomain->maxCardsOfEachType; ++j) {
        for (int i = 0; i < pokerDomain->maxCardTypes; ++i) {
          ...
          newLastAction = a1;
          next_players[0] = 0;
          newState = make_shared<RhodeIslandPokerState>(domain, player1Card_, player2Card_, make_pair(i,j),
                                                        nullopt,
          ...
          Outcome outcome(newState, vector<shared_ptr<Observation>>{
            make_shared<RhodeIslandPokerObservation>(3 + i+j*pokerDomain->maxCardTypes, PlayCard, i, j),
                  make_shared<RhodeIslandPokerObservation>(3 + i+j*pokerDomain->maxCardTypes, PlayCard, i, j)}, vector<double>(2));
    ...
    newState = make_shared<RhodeIslandPokerState>(domain, player1Card_, player2Card_, natureCard1_, natureCard2_,
                                              newFirstPlayerReward, new_pot, next_players, new_round, newLastAction, newContinuousRaiseCount);
    observations[0] = make_shared<RhodeIslandPokerObservation>(-1,-1, -1, -1);
    observations[1] = make_shared<RhodeIslandPokerObservation>(id, a1->GetType(), a1->GetValue(), 0);
      ...
      case Raise:
        ...
        switch (round_) {
          case 1:
            ...
            break;
          case 3:
            raisePosition = static_cast<int>(pokerDomain->maxDifferentRaises);
            for(auto &i : pokerDomain->raisesSecondRound){
              if(a2->GetValue() == i) {
                break;
              }
            ...
            break;
          case 5:
            raisePosition = 2*static_cast<int>(pokerDomain->maxDifferentRaises);
            for(auto &i : pokerDomain->raisesThirdRound){
              if(a2->GetValue() == i) {
                break;
              }
      ...
      case Bet:
        ...
        switch(round_)  {
          case 1:
            ...
            break;
          case 3:
            betPosition = static_cast<int>(pokerDomain->maxDifferentBets);
            for(auto &i : pokerDomain->betsSecondRound){
              if(bet == i) {
                break;
              }
            ...
            break;
          case 5:
            betPosition = 2*static_cast<int>(pokerDomain->maxDifferentBets);
            for(auto &i : pokerDomain->betsSecondRound){
              if(bet == i) {
                break;
              }
    ...
    if (new_round == 2 && natureCard1_ == nullopt && (a2->GetType() == Call || a2->GetType() == Check)) {
      double prob = 1.0/(pokerDomain->maxCardsOfEachType * pokerDomain->maxCardTypes - 2);
      for(int j = 0; j < pokerDomain->maxCardsOfEachType; ++j) {
        for (int i = 0; i < pokerDomain->maxCardTypes; ++i) {
          if ((player1Card_.first == i && player1Card_.second == j)
          ...
          next_players[0] = 0;
          newState = make_shared<RhodeIslandPokerState>(domain, player1Card_, player2Card_, make_pair(i,j),
                                                        nullopt, newFirstPlayerReward, new_pot,
                                                        next_players, new_round + 1, newLastAction, 0);
          Outcome outcome(newState, vector<shared_ptr<Observation>>{
                  make_shared<RhodeIslandPokerObservation>(3 + i+j*pokerDomain->maxCardTypes, PlayCard, i, j),
                  make_shared<RhodeIslandPokerObservation>(3 + i+j*pokerDomain->maxCardTypes, PlayCard, i, j)}, vector<double>(2));
    ...
    } else if (new_round == 4 && natureCard2_ == nullopt && (a2->GetType() == Call || a2->GetType() == Check)) {
      double prob = 1.0/(pokerDomain->maxCardsOfEachType * pokerDomain->maxCardTypes - 3);
      for(int j = 0; j < pokerDomain->maxCardsOfEachType; ++j) {
        for (int i = 0; i < pokerDomain->maxCardTypes; ++i) {
          if ((player1Card_.first == i && player1Card_.second == j)
          ...
          newState = make_shared<RhodeIslandPokerState>(domain, player1Card_, player2Card_,
                                                        natureCard1_, make_pair(i,j), newFirstPlayerReward, new_pot,
                                                        next_players, new_round + 1, newLastAction, 0);
          Outcome outcome(newState, vector<shared_ptr<Observation>>{
                  make_shared<RhodeIslandPokerObservation>(3 + i+j*pokerDomain->maxCardTypes, PlayCard, i, j),
                  make_shared<RhodeIslandPokerObservation>(3 + i+j*pokerDomain->maxCardTypes, PlayCard, i, j)}, vector<double>(2));
    ...
    newState = make_shared<RhodeIslandPokerState>(domain, player1Card_, player2Card_, natureCard1_, natureCard2_,
            newFirstPlayerReward, new_pot, next_players, new_round, newLastAction, newContinuousRaiseCount);
    observations[0] = make_shared<RhodeIslandPokerObservation>(id, a2->GetType(), a2->GetValue(), 0);
    observations[1] = make_shared<RhodeIslandPokerObservation>(-1,-1,-1, -1);
  }
  ...
  if(new_round == pokerDomain->TERMINAL_ROUND) { // TODO: check it
    if(newLastAction->GetType() == Fold) {
      rewards = a1? vector{-newFirstPlayerReward, newFirstPlayerReward} : vector{newFirstPlayerReward, -newFirstPlayerReward};
    ...
    } else if (player1Card_.first == natureCard1_.value().first || player1Card_.first == natureCard2_.value().first) {
      rewards = vector<double>{newFirstPlayerReward, -newFirstPlayerReward};
    } else if (player2Card_.first == natureCard1_.value().first || player2Card_.first == natureCard2_.value().first) {
      rewards = vector<double>{-newFirstPlayerReward, newFirstPlayerReward};
...
RhodeIslandPokerState::RhodeIslandPokerState(Domain* domain, pair<int, int> p1card,
        pair<int, int> p2card, optional<pair<int, int>> natureCard1, optional<pair<int, int>> natureCard2,
        ...
        State(domain), player1Card_(move(p1card)), player2Card_(move(p2card)), natureCard1_(move(natureCard1)),
        natureCard2_(move(natureCard2)), pot(pot), firstPlayerReward(firstPlayerReward),
