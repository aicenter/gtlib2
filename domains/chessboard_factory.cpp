...
namespace chess {
    ...
    boardInfo FenBoardFactory::create(string s, GTLib2::domains::KriegspielState* b) {
        ...
        if(strs[2] == "----" || strs[2] == "--") {
            ...
            for(shared_ptr<AbstractPiece> r: rooks) {
                ...
                if(r->getPosition().x != 1 || r->getPosition().x != binfo.x || r->getPosition().y != 1 || r->getPosition().y != binfo.y) {
                    r->setHasMoved(true);
    ...
    shared_ptr<AbstractPiece> FenBoardFactory::createPiece(char & c, int x, int y, GTLib2::domains::KriegspielState* b) {
        char pieceChar = tolower(c);
        Square s(x, y);
        switch(pieceChar) {
            case 'r':
                return make_shared<Rook>(static_cast<pieceName>(pieceChar), isupper(c) ? chess::WHITE : chess::BLACK, s, b);
            case 'q':
                return make_shared<Queen>(static_cast<pieceName>(pieceChar), isupper(c) ? chess::WHITE : chess::BLACK, s, b);
            case 'k':
                return make_shared<King>(static_cast<pieceName>(pieceChar), isupper(c) ? chess::WHITE : chess::BLACK, s, b);
            case 'n':
                return make_shared<Knight>(static_cast<pieceName>(pieceChar), isupper(c) ? chess::WHITE : chess::BLACK, s, b);
            case 'b':
