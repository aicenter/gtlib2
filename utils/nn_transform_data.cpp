...
TransformCFRDataToNN::TransformCFRDataToNN(
    ...
    this->information_set_mapping_ = information_set_mapping;

    this->augmented_information_set_mapping_ = augmented_information_set_mapping;

    this->features_public_state_ = features_public_state;

    this->number_of_rounds_ = this->features_public_state_.at(0).size();
    ...
    this->input_nn_matrix_dim_x_ =
            this->number_of_rounds_ + num_possible_actions_sequences_p0 + num_possible_actions_sequences_p1;
    this->input_nn_matrix_dim_y_ = features_public_state.size();

    this->output_nn_matrix_dim_x_ = num_possible_actions_sequences_p0 + num_possible_actions_sequences_p1;
    this->output_nn_matrix_dim_y_ = features_public_state.size();
...
vector<vector<double>> TransformCFRDataToNN::to_nn_input(const vector<double> &range_acting_player,
    ...
    for (int y = 0; y < this->input_nn_matrix_dim_y_; y++) {
        vector<double> row_vector(this->input_nn_matrix_dim_x_, 0);
    ...
    for (int y = 0; y < this->input_nn_matrix_dim_y_; y++) {
        vector<double> feature_public_state = this->features_public_state_.at(y);

        for (int x = 0; x < this->number_of_rounds_; x++) {
            ...
            if (information_set_mapping_[y][x]) {
                ...
                for (unsigned int z = 0; z < z_dimension; z++) {
                    ...
                    if (this->is_reachable(range_acting_player[index])) {
                        returning_matrix.at(y).at(x + this->number_of_rounds_) = range_acting_player[index];
                        break;
                    }
                }
            }
            ...
            if (augmented_information_set_mapping_[y][x]) {
                ...
                for (unsigned int z = 0; z < z_dimension; z++) {
                    ...
                    if (this->is_reachable(range_opponent_player[index])) {
                        returning_matrix.at(y).at(x + this->number_of_rounds_ +
