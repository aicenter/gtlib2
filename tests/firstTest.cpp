...
TEST_F(FTest, MMDepth2PROP33) {
  ...
  ASSERT_EQ(countStates, 24);  //  MMPursuitState::PerformAction: count = 2; each player has one move, then plays the second one
...
TEST_F(FTest, MMDepth3PROP44) {
  ...
  ASSERT_EQ(reward[0], 0);
  ASSERT_EQ(reward[1], 0);
  ASSERT_EQ(countStates, 132);  //  MMPursuitState::PerformAction: count = 2; each player has one move, then plays the second one
...
TEST_F(FTest, MMDepth4PROP44) {
  ...
  ASSERT_EQ(countStates, 696);  //  MMPursuitState::PerformAction: count = 2; each player has one move, then plays the second one
